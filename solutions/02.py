...
class Solution:
    ...
    def _preprocess_input(self, data_input: list) -> list:
        data_list = []
        for entry in data_input:
            data = entry.split(" ")
            num_range = data[0].split("-")
            data_list.append((num_range[0], num_range[1], data[1][0], data[2]))

        return data_list

    def check_password_valid_part_1(self) -> int:
        preprocessed_input = self._preprocess_input(INPUT)
        valid_count = 0
        for entry in preprocessed_input:
            count_min = int(entry[0])
            count_max = int(entry[1])
            char = entry[2]
            pw = entry[3]
            char_count = 0
            ...
            for c in pw:
                if c == char:
                    char_count += 1
                if char_count > count_max:
                    break

            if count_min <= char_count <= count_max:
                valid_count += 1

        return valid_count

    def check_password_valid_part_2(self) -> int:
        preprocessed_input = self._preprocess_input(INPUT)
        valid_count = 0
        for entry in preprocessed_input:
            pos1 = int(entry[0]) - 1
...
print(Solution().check_password_valid_part_1())
print(Solution().check_password_valid_part_2())
